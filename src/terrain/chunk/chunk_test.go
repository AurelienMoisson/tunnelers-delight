...
func TestStability(t *testing.T) {
    ...
    chunk.SetStability(-1, 2, 1, MaxStability)
    if chunk.GetStability(-1, 2, 1) != MaxStability {
        t.Error("SetStability didn't work")
    }
    chunk.SetBlock(0, 2, 1, block1)
    if chunk.GetStability(0, 2, 1) != MaxStability-20 {
        log.Print("found error of ", chunk.GetStability(0, 2, 1), " instead of ", MaxStability-20)
        t.Error("stability did not propagate correctly horrizontally")
    }
    chunk.SetBlock(0, 2, 1, air)
    chunk.SetBlock(1,2,1, block1)
    if chunk.GetStability(1,2,1) > 0 {
        t.Error("positive stability for floating block")
    }
    chunk.SetBlock(0,2,1, block1)
    if chunk.GetStability(1,2,1) != MaxStability-40 {
...
func BenchmarkStability(b *testing.B) {
    ...
    chunk.SetBlock(0,1,2, block1)
    chunk.SetBlock(7,8,1, block1)
    chunk.SetBlock(7,8,0, block1)
    b.ResetTimer()
    for i:=0; i<b.N; i++ {
